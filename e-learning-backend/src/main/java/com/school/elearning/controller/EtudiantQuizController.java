...
public class EtudiantQuizController {
    ...
    // ── SOUMETTRE LA TENTATIVE (Manuellement ou fin de temps) ──
    // POST /api/etudiant/quiz/{tentativeId}/soumettre
    @PostMapping("/quiz/{tentativeId}/soumettre")
    public ResponseEntity<ScoreFinaleResponse> soumettreTentative(
            @PathVariable Long tentativeId, Authentication auth) {
        return ResponseEntity.ok(etudiantQuizService.soumettreTentative(tentativeId, auth));
    }
