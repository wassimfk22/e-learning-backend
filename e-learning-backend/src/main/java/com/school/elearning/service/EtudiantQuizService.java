...
public class EtudiantQuizService {
    ...
    private ReponseQuizResultat traiterEnregistrementReponse(ReponseQuizRequest req, TentativeQuiz tentative) {
        ...
        if (reponseRepository.existsByTentativeAndQuestion(tentative, question)) {
            // On peut aussi choisir de mettre à jour la réponse existante au lieu d'une erreur
        ...
        return transformerEnResultat(reponse, tentative);
    }

    private ReponseQuizResultat transformerEnResultat(ReponseQuiz reponse, TentativeQuiz tentative) {
        ReponseQuizResultat res = new ReponseQuizResultat();
        res.setQuestionId(reponse.getQuestion().getId());
        res.setReponseChoisie(reponse.getReponseChoisie());
        res.setEstCorrecte(reponse.isEstCorrecte());
        res.setPointsObtenus(reponse.getPointsObtenus());
        res.setBonneReponse(reponse.getQuestion().getBonneReponse());
        res.setQuestionsRepondues(reponseRepository.countByTentative(tentative));
        res.setTotalQuestions(tentative.getQuiz().getQuestions().size());
        res.setToutesReponsesEnvoyees(false);
        return res;
    }
    ...
    private void verifierEtFinaliserTentative(TentativeQuiz tentative, List<ReponseQuizResultat> resultats) {
        ...
        if (questionsRepondues >= totalQuestions) {
            ...
            if (pointsTotauxObtenus == null) pointsTotauxObtenus = 0.0;

            double baremeMaximum = tentative.getQuiz().getQuestions().stream()
                                            .mapToDouble(QuestionQuiz::getPoints).sum();

            // Calcul du pourcentage : (Obtenu / Total) * 100
            double pourcentage = baremeMaximum > 0 ? (pointsTotauxObtenus / baremeMaximum) * 100 : 0;

            // Mise à jour de la tentative
            tentative.setScoreObtenu(pointsTotauxObtenus);
            tentative.setScoreMax(baremeMaximum);
            ...
            // Mettre à jour les résultats et la progression
            mettreAJourResultatEtProgression(tentative);

            // On informe le dernier résultat de la liste avec un ScoreFinaleResponse
            ScoreFinaleResponse scoreFinale = new ScoreFinaleResponse();
            scoreFinale.setTentativeId(tentative.getId());
            scoreFinale.setNumeroTentative(tentative.getNumeroTentative());
            scoreFinale.setScoreObtenu(pointsTotauxObtenus);
            scoreFinale.setScoreMax(baremeMaximum);
            scoreFinale.setPourcentage(Math.round(pourcentage * 100.0) / 100.0);
            scoreFinale.setTentativesRestantes(tentative.getTentativesRestantes());
            scoreFinale.setTentativesEpuisees(tentative.getTentativesRestantes() == 0);
            scoreFinale.setDateSoumission(tentative.getDateSoumission());
            scoreFinale.setMessage(genererMessage(pourcentage, tentative.getTentativesRestantes()));

            ReponseQuizResultat dernierRes = resultats.get(resultats.size() - 1);
            dernierRes.setToutesReponsesEnvoyees(true);
            dernierRes.setScoreFinale(scoreFinale);
        }
    }
    ...
    @Transactional
    public ScoreFinaleResponse soumettreTentative(Long tentativeId, Authentication auth) {
        Etudiant etudiant = getEtudiantConnecte(auth);
        TentativeQuiz tentative = tentativeRepository.findById(tentativeId)
                .orElseThrow(() -> new RuntimeException("Tentative introuvable"));
        ...
        if (scoreObtenu == null) scoreObtenu = 0.0;
        
        double scoreMax = tentative.getQuiz().getQuestions().stream()
                .mapToDouble(QuestionQuiz::getPoints).sum();
        ...
        // Message dynamique
        double pourcentage = scoreMax > 0 ? (scoreObtenu / scoreMax) * 100 : 0;
        String message = genererMessage(pourcentage, tentative.getTentativesRestantes());
        ...
        score.setTentativesRestantes(tentative.getTentativesRestantes());
        score.setTentativesEpuisees(tentative.getTentativesRestantes() == 0);
